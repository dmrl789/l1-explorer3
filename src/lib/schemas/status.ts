...
export function normalizeStatus(raw: unknown): Status {
  ...
  const normalized: Record<string, unknown> = {
    ...
    latest_round_id: data.latest_round_id ?? data.head_round ?? consensus?.round ?? data.round ?? data.tip_round_id,
    latest_block_id: data.latest_block_id ?? data.head_block ?? data.block ?? data.tip_block_hash,
    latest_hashtimer: data.latest_hashtimer ?? data.head_hashtimer ?? data.hashtimer ?? data.current_hashtimer,
    // Normalize TPS
    ...
    // Normalize validators - backend uses validator_count
    active_validators: data.active_validators ?? data.validators ?? data.validator_count ?? consensus?.validator_count ?? 0,
    shadow_verifiers: data.shadow_verifiers ?? data.verifiers ?? 0,
    total_nodes: data.total_nodes ?? data.nodes ?? data.peer_count ?? 0,
    // Version info from build object
    version: data.version ?? (data.build as Record<string, unknown>)?.git,
    commit: data.commit ?? data.build_sha,
